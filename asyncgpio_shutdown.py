...
class PinWatcher:
    ...
    async def _pin_monitor(self, shutdown_manager):
...
class PinFollower:
    ...
    async def set_pin_level(self, pin_watcher, shutdown_manager):
...
async def monitor_pin_loop(pin_watcher, shutdown_manager):
...
async def func_on_trigger(pin_watcher, shutdown_manager):
...
async def func_off_trigger(pin_watcher, shutdown_manager):
...
async def main():
    ...
    async with open_nurseries() as nurseries:
        ...
        nursery.start_soon(in_pin._pin_monitor, shutdown_manager)
        nursery.start_soon(func_off_trigger, in_pin, shutdown_manager)
        nursery.start_soon(func_on_trigger, in_pin, shutdown_manager)
        nursery.start_soon(pin_mirror.set_pin_level, in_pin, shutdown_manager)
        nursery.start_soon(pin_invert.set_pin_level, in_pin, shutdown_manager)
        nursery.start_soon(monitor_pin_loop, in_pin, shutdown_manager)
